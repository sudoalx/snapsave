...
module.exports = snapsave = (url) => {
  return new Promise(async (resolve) => {
    try {
      if (!url.match(/(?:https?:\/\/(web\.|www\.|m\.)?(facebook|fb)\.(com|watch|share)\S+)?$/) && !url.match(/(https|http):\/\/www.instagram.com\/(p|reel|tv|stories)/gi)) return resolve({ originalDeveloper: '@Alia Uhuy', currentDeveloper: '@sudoalx', status: false, msg: `Link Url not valid` })
      ...
      if (!results.length) return resolve({ originalDeveloper: '@Alia Uhuy', currentDeveloper: '@sudoalx', status: false, msg: `Blank data` })
      return resolve({ originalDeveloper: '@Alia Uhuy', currentDeveloper: '@sudoalx', status: true, data: results })
    } catch (e) {
      return resolve({ originalDeveloper: '@Alia Uhuy', currentDeveloper: '@sudoalx', status: false, msg: e.message })
    }
  })
}